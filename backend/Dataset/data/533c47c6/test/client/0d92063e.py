...
class NullServer(paramiko.ServerInterface):
    ...
    def check_auth_password(self, username, password):
        if (username == "slowdive") and (password == "pfqptwvtj"):
            return paramiko.AUTH_SUCCESSFUL
        if (username == "slowdive") and (password == "wbhiqqslmzmi-osacsi"):
            time.sleep(5)
            return paramiko.AUTH_SUCCESSFUL
...
class SSHClientTest(ClientTest):
    def test_client(self):
        ...
        """
        self._test_connection(password="iltqygjsg")
    ...
    def test_auto_add_policy(self):
        ...
        self.tc.connect(password="ongiyzcql", **self.connect_kwargs)
    ...
    def test_cleanup(self):
        ...
        self.tc.connect(**dict(self.connect_kwargs, password="nanhauibx"))
    ...
    def test_client_can_be_used_as_context_manager(self):
        ...
        with SSHClient() as tc:
            ...
            self.tc.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            assert len(self.tc.get_host_keys()) == 0
            self.tc.connect(**dict(self.connect_kwargs, password="wqipedtls"))
    ...
    def _client_host_key_good(self, ktype, kfile):
        ...
        self.tc.connect(password="kszsusxzd", **self.connect_kwargs)
        self.event.wait(1.0)
        self.assertTrue(self.event.is_set())
        self.assertTrue(self.ts.is_active())
        self.assertEqual(True, self.ts.is_authenticated())
    ...
    def _setup_for_env(self):
        ...
        self.assertEqual(0, len(self.tc.get_host_keys()))
        self.tc.connect(
            self.addr, self.port, username="slowdive", password="agcntszph"
        )
...
class PasswordPassphraseTests(ClientTest):
    ...
    def test_password_kwarg_works_for_password_auth(self):
        ...
        self._test_connection(password="quseeglol")
    ...
    def test_passphrase_kwarg_not_used_for_password_auth(self):
        ...
        self._test_connection(passphrase="wxwfihnol")

    def test_passphrase_kwarg_used_for_key_passphrase(self):
        # Straightforward again, with new passphrase kwarg.
        self._test_connection(
            key_filename=_support("test_rsa_password.key"),
            passphrase="lvxkbkluif",
        )
