...
compiled Apr  5 2019, 18:59:58 on x86_64-apple-darwin17.0.0"""
    ...
    def test_credentials(self):
        svn = SVN(folder=self.gimme_tmp(), username="ada", password="sgkfhili")
    ...
    def test_verify_ssl(self):
        ...
        svn = SVN(folder=self.gimme_tmp(), username="peter", password="xulhj", verify_ssl=True)
        runner = MyRunner(svn)
        svn.checkout(url=project_url)
        self.assertNotIn("--trust-server-cert-failures=unknown-ca", runner.calls[1])

        svn = SVN(folder=self.gimme_tmp(), username="peter", password="xesau", verify_ssl=False)
        runner = MyRunner(svn)
        svn.checkout(url=project_url)
