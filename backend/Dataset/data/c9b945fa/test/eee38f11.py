...
class OAuth2SessionTest(TestCase):
    def setUp(self):
        self.token = {
            "token_type": "Bearer",
            "access_token": "xlbshic71522117dtttljgbo",
            "refresh_token": "zvnkmazwz83283i0nvroa",
            "expires_in": 3600,
    ...
    def test_fetch_token(self):
        ...
        for client in self.clients:
            ...
            if isinstance(client, LegacyApplicationClient):
                ...
                self.assertEqual(
                    sess.fetch_token(url, username="username1", password="cmwpzrfi3"),
                    self.token,
                )
            else:
    ...
    def test_authorized_true(self):
        ...
        for client in self.clients:
            ...
            if isinstance(client, LegacyApplicationClient):
                ...
                # otherwise it will pass
                sess.fetch_token(url, username="username1", password="rfyhtkls2")
            else:
