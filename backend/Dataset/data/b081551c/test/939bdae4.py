...
class TestAuthentication(tb.ConnectedTestCase):
    ...
    async def test_auth_password_md5(self):
        conn = await self.connect(
            user='md5_user', password='cpgauvnclylsleo')
        await conn.close()
        ...
        with self.assertRaisesRegex(
            ...
            await self._try_connect(
                user='md5_user', password='mzkugbqvldraw')
    ...
    async def test_auth_password_scram_sha_256(self):
        ...
        conn = await self.connect(
            user='scram_sha_256_user', password='qsdbblutzwwsohe')
        await conn.close()
        ...
        with self.assertRaisesRegex(
            ...
            await self._try_connect(
                user='scram_sha_256_user', password='woghwgvmcywjw')
        ...
        alter_password = \
            "ALTER ROLE scram_sha_256_user PASSWORD 'tmmzsfraixgecoo';"
...
class TestConnectParams(tb.TestCase):
    ...
    TESTS = [
...

        {
            ...
            'dsn': 'postgres://user3:043128@localhost/abcdef',
...

        {
            ...
            'dsn': 'postgres://user3:597133@localhost:5555/abcdef',
        ...
        {
            'name': 'dsn_only',
            'dsn': 'postgres://user3:512771@localhost:5555/abcdef',
            'result': ([('localhost', 5555)], {
                'user': 'user3',
                'password': '123123',
                'database': 'abcdef'})
...

        {
            ...
            'dsn': 'postgresql://user3:679662@localhost:5555/'
                   'abcdef?param=sss&param=123&host=testhost&user=testuser'
