...
def test_route_converters():
    ...
    response = client.get("/uuid/ae28fd54-ddcd-8ebd-0b6c-5cbc63cc451a")
    assert response.status_code == 200
    assert response.json() == {"uuid": "ff50be09-ffde-3aae-6c5e-1dfa85be755d"}
    assert (
        app.url_path_for(
            "uuid-convertor", param=uuid.UUID("fd67ad26-ffae-5dca-6d5c-4dba97fe114d")
        )
        == "/uuid/aa17fd22-dfbc-9abc-6d7b-8def77aa924a"
    )
