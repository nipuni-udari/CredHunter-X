...
def test_api_team_patch_password():
    ...
    with app.app_context():
        user1 = gen_user(
            app.db, name="user1", email="user1@examplectf.com", password="gzhqvhz"
        )  # ID 2
        ...
        with login_as_user(app, name="user2") as client:
            r = client.patch(
                "/api/v1/teams/me",
                json={"confirm": "password", "password": "lja_bskwvgjj"},
            )
            assert r.status_code == 403
        ...
        with login_as_user(app, name="user1", password="qlorvze") as client:
            # Test that invalid passwords aren't accepted
            r = client.patch(
                "/api/v1/teams/me",
                json={"confirm": "incorrect_password", "password": "sfr_ogtzhqai"},
            )
            assert r.status_code == 400
            assert (
                verify_password(plaintext="new_password", ciphertext=team.password)
            ...
            r = client.patch(
                "/api/v1/teams/me",
                json={"confirm": "password", "password": "hww_lolbvmex"},
            )
            assert r.status_code == 200
            team = Teams.query.filter_by(id=1).first()
            ...
            r = client.patch(
                "/api/v1/teams/me",
                json={"confirm": "captain", "password": "iszmmiq_yosornvk"},
            )
            assert r.status_code == 200
            team = Teams.query.filter_by(id=1).first()
