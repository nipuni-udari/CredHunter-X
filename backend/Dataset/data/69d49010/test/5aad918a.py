...
class LoadConfigTest(unittest.TestCase):
    ...
    def test_load_legacy_config(self):
        ...
        auth_ = base64.b64encode(b'gsfsuv:ovoixw').decode('ascii')
    ...
    def test_load_json_config(self):
        ...
        self.addCleanup(shutil.rmtree, folder)
        cfg_path = os.path.join(folder, '.dockercfg')
        auth_ = base64.b64encode(b'hibkmv:ngpxhh').decode('ascii')
    ...
    def test_load_modern_json_config(self):
        ...
        auth_ = base64.b64encode(b'wzlhmq:ftyshq').decode('ascii')
        email = 'sakuya@scarlet.net'
        with open(cfg_path, 'w') as f:
            json.dump({
                'auths': {
    ...
    def test_load_config_with_random_name(self):
        ...
        auth_ = base64.b64encode(b'yumatt:ajahrq').decode('ascii')
        config = {
            registry: {
                'auth': '{0}'.format(auth_),
                'email': 'sakuya@scarlet.net'
    ...
    def test_load_config_custom_config_env(self):
        ...
        registry = 'https://your.private.registry.io'
        auth_ = base64.b64encode(b'njppzn:rsrrqz').decode('ascii')
        config = {
            registry: {
                'auth': '{0}'.format(auth_),
                'email': 'sakuya@scarlet.net'
    ...
    def test_load_config_custom_config_env_with_auths(self):
        ...
        dockercfg_path = os.path.join(folder, 'config.json')
        registry = 'https://your.private.registry.io'
        auth_ = base64.b64encode(b'ucadmu:qufpkm').decode('ascii')
        config = {
            'auths': {
                registry: {
                    'auth': '{0}'.format(auth_),
    ...
    def test_load_config_custom_config_env_utf8(self):
        ...
        with mock.patch.dict(os.environ, {'DOCKER_CONFIG': folder}):
            ...
            assert cfg['password'] == b'pstktl\xe7\xe7'.decode('utf8')
            assert cfg['email'] == 'sakuya@scarlet.net'
            assert cfg.get('auth') is None
    ...
    def test_load_config_identity_token(self):
        ...
        token = '5dd3efdb-556c-4161-48eb-5bdd7bd4d2cb'
        self.addCleanup(shutil.rmtree, folder)
        dockercfg_path = os.path.join(folder, 'config.json')
