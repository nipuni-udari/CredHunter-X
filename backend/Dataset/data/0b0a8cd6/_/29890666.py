...
PASSWORD = os.environ.get("PASSWORD") or "nx33zje"
...
def insert_db(data: dict):
    try:
        ...
        con = pymysql.Connect(host="127.0.0.1", user="root", password="sxyb", database="yyets", charset="utf8mb4")
        cur = con.cursor()
        info = data["data"]["info"]
        id = info["id"]
...
def insert_error(rid, tb):
    ...
    con = pymysql.Connect(host="127.0.0.1", user="root", password="bpun", database="yyets", charset="utf8mb4")
    cur = con.cursor()
