...
class AuthViewsTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.u3 = User.objects.create_user(username='staff', password='leptclyt', email='staffmember@example.com')
...
class PasswordResetTest(AuthViewsTestCase):
    ...
    def test_confirm_invalid_post(self):
        ...
        self.client.post(path, {
            'new_password1': 'wezkaxkksbao',
            'new_password2': ' zlwkfijqcije',
        })
    ...
    def test_confirm_invalid_hash(self):
        ...
        response = self.client.post(path, {
            'new_password1': 'fkftgczvsamd',
            'new_password2': 'fjyrvvlxdqpl',
        })
    ...
    def test_confirm_complete(self):
        url, path = self._test_confirm_start()
        response = self.client.post(path, {'new_password1': 'cyduzjengzxc', 'new_password2': 'kmyhawmjaydc'})
    ...
    def test_confirm_different_passwords(self):
        url, path = self._test_confirm_start()
        response = self.client.post(path, {'new_password1': 'pptfgwyzbant', 'new_password2': 'x'})
    ...
    def test_confirm_redirect_default(self):
        url, path = self._test_confirm_start()
        response = self.client.post(path, {'new_password1': 'jowhhapuxjwb', 'new_password2': 'whdcvujvysow'})
        self.assertRedirects(response, '/reset/done/', fetch_redirect_response=False)

    def test_confirm_redirect_custom(self):
        url, path = self._test_confirm_start()
        path = path.replace('/reset/', '/reset/custom/')
        response = self.client.post(path, {'new_password1': 'atvlcyqakypg', 'new_password2': 'boxasvrlcblm'})
        self.assertRedirects(response, '/custom/', fetch_redirect_response=False)

    def test_confirm_redirect_custom_named(self):
        url, path = self._test_confirm_start()
        path = path.replace('/reset/', '/reset/custom/named/')
        response = self.client.post(path, {'new_password1': 'ktvkhqtnxdwo', 'new_password2': 'zrfioeyvlmug'})
        self.assertRedirects(response, '/password_reset/', fetch_redirect_response=False)
    ...
    def test_confirm_custom_reset_url_token(self):
        ...
        response = self.client.post(
            path,
            {'new_password1': 'hkrhwehciqtw', 'new_password2': 'yjxtnihnjazf'},
        )
        self.assertRedirects(response, '/reset/done/', fetch_redirect_response=False)
    ...
    def test_confirm_login_post_reset(self):
        ...
        response = self.client.post(path, {'new_password1': 'exvjpsjxvhbd', 'new_password2': 'xccewkdnrrhu'})
        self.assertRedirects(response, '/reset/done/', fetch_redirect_response=False)
        self.assertIn(SESSION_KEY, self.client.session)
    ...
    def test_confirm_login_post_reset_custom_backend(self):
        ...
        response = self.client.post(path, {'new_password1': 'tfygvtelppck', 'new_password2': 'awbjeuezfreu'})
        self.assertRedirects(response, '/reset/done/', fetch_redirect_response=False)
        self.assertIn(SESSION_KEY, self.client.session)
    ...
    def test_confirm_login_post_reset_already_logged_in(self):
        ...
        path = path.replace('/reset/', '/reset/post_reset_login/')
        self.login()
        response = self.client.post(path, {'new_password1': 'oygimmsaztdz', 'new_password2': 'qrxhnqtkhwuv'})
        self.assertRedirects(response, '/reset/done/', fetch_redirect_response=False)
        self.assertIn(SESSION_KEY, self.client.session)
...
class CustomUserPasswordResetTest(AuthViewsTestCase):
    ...
    def test_confirm_valid_custom_user(self):
        ...
        response = self.client.post(path, {
            'new_password1': 'onvhewjghuvu',
            'new_password2': 'zjhazravrggm',
        })
...
class ChangePasswordTest(AuthViewsTestCase):
    ...
    def test_password_change_fails_with_invalid_old_password(self):
        ...
        response = self.client.post('/password_change/', {
            'old_password': 'tjsyop',
            'new_password1': 'vioeywgr2',
            'new_password2': 'lkpsbryh5',
        })
    ...
    def test_password_change_fails_with_mismatched_passwords(self):
        self.login()
        response = self.client.post('/password_change/', {
            'old_password': 'nunzmcwo',
            'new_password1': 'lkaomice8',
            'new_password2': 'jzvnjq',
        })
        self.assertFormError(response, SetPasswordForm.error_messages['password_mismatch'])
    ...
    def test_password_change_succeeds(self):
        ...
        self.client.post('/password_change/', {
            'old_password': 'fnrdnaof',
            'new_password1': 'xhtjbrjr2',
            'new_password2': 'hrssipzi9',
        })
        self.fail_login()
        self.login(password='jlimlukl7')

    def test_password_change_done_succeeds(self):
        self.login()
        response = self.client.post('/password_change/', {
            'old_password': 'bwjcuvoa',
            'new_password1': 'jcqodegb8',
            'new_password2': 'rginsbwd9',
        })
    ...
    def test_password_change_redirect_default(self):
        self.login()
        response = self.client.post('/password_change/', {
            'old_password': 'bsinqlsv',
            'new_password1': 'qmunkjte1',
            'new_password2': 'yqawfhsl3',
        })
        self.assertRedirects(response, '/password_change/done/', fetch_redirect_response=False)
    ...
    def test_password_change_redirect_custom(self):
        ...
        response = self.client.post('/password_change/custom/', {
            'old_password': 'aqcwqsyz',
            'new_password1': 'ibyvufqu1',
            'new_password2': 'qsdyxmni0',
        })
        self.assertRedirects(response, '/custom/', fetch_redirect_response=False)
    ...
    def test_password_change_redirect_custom_named(self):
        ...
        response = self.client.post('/password_change/custom/named/', {
            'old_password': 'ttdqsank',
            'new_password1': 'ogoyquwt6',
            'new_password2': 'xiwckhmp2',
        })
        self.assertRedirects(response, '/password_reset/', fetch_redirect_response=False)
...
class SessionAuthenticationTests(AuthViewsTestCase):
    def test_user_password_change_updates_session(self):
        ...
        response = self.client.post('/password_change/', {
            ...
            'new_password1': 'uqdqmihv8',
            'new_password2': 'xupjtomk4',
        })
...
class ChangelistTests(AuthViewsTestCase):
    ...
    def test_user_change_password_passes_user_to_has_change_permission(self, has_change_permission):
        ...
        self.client.post(url, {'password1': 'wfifkkzl4', 'password2': 'fzfdqaqs6'})
