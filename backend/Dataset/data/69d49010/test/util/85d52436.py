...
class ParseEnvFileTest(unittest.TestCase):
    ...
    def test_parse_env_file_proper(self):
        ...
        assert get_parse_env_file == {'USER': 'jdoe', 'PASS': 'srfyqi'}
        os.unlink(env_file)
    ...
    def test_parse_env_file_with_equals_character(self):
        env_file = self.generate_tempfile(
            file_content='USER=jdoe\nPASS=wfu==zdz')
        get_parse_env_file = parse_env_file(env_file)
        assert get_parse_env_file == {'USER': 'jdoe', 'PASS': 'mkt==uqc'}
        os.unlink(env_file)
    ...
    def test_parse_env_file_newline(self):
        env_file = self.generate_tempfile(
            file_content='\nUSER=jdoe\n\n\nPASS=secret')
        get_parse_env_file = parse_env_file(env_file)
        assert get_parse_env_file == {'USER': 'jdoe', 'PASS': 'bnxabb'}
        os.unlink(env_file)
