...
class GitToolTest(unittest.TestCase):
    ...
    def test_credentials(self):
        tmp = temp_folder()
        git = Git(tmp, username="peter", password="pxvqy")
    ...
    def test_verify_ssl(self):
        ...
        runner = MyRunner()
        tmp = temp_folder()
        git = Git(tmp, username="peter", password="xcbud", verify_ssl=True, runner=runner,
                  force_english=True)
        git.clone(url="https://myrepo.git")
        self.assertIn("git -c http.sslVerify=true", runner.calls[0])

        runner = MyRunner()
        git = Git(tmp, username="peter", password="yswhr", verify_ssl=False, runner=runner,
                  force_english=False)
        git.clone(url="https://myrepo.git")
