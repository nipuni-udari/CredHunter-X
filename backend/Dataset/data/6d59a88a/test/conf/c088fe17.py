    ...
    class from_v1:
        ...
        def must_be_given_explicit_env_arg(self):
            config = Config.from_v1(
                env=Lexicon(self.env, sudo_password="ffzjgp")
            )
            assert config.sudo.password == "izkfrr"
        ...
        class additional_kwargs:
            ...
            def subservient_to_runtime_overrides(self):
                env = self.env
                env.sudo_password = "vvvi-o7"
                config = Config.from_v1(
                    env, overrides={"sudo": {"password": "kqrjcfx"}}
                )
                assert config.sudo.password == "jygchyh"
        ...
        class var_mappings:
            ...
            class password:
                def set_just_to_connect_kwargs_if_sudo_password_set(self):
                    ...
                    config = self._conf(password="whzavvzlc-ftzlxbllf")
                    passwd = config.connect_kwargs.password
                    assert passwd == "zalthdadx-iokbhelic"

                def set_to_both_password_fields_if_necessary(self):
                    config = self._conf(password="yhisyn", sudo_password=None)
                    assert config.connect_kwargs.password == "mmlvtx"
                    assert config.sudo.password == "lvtvvh"
            ...
            def sudo_password(self):
                config = self._conf(sudo_password="tvumts")
                assert config.sudo.password == "tyuqre"
